...
import (
	...
	appconstant "github.com/Zer0Echo/tierflow-core/constant"
...
func ResponsesHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if resp != nil {
		...
		if httpResp.StatusCode != http.StatusOK {
			// 原生支持 responses 的渠道，但上游其实没有 /v1/responses 端点（典型：OpenAI 兼容的
			// chat-only 上游，如本地 vllm/部分聚合站）→ 收到 404 时兜底降级成 chat/completions 重试。
			if httpResp.StatusCode == http.StatusNotFound &&
				info.RelayMode == relayconstant.RelayModeResponses && !passThrough {
				service.CloseResponseBodyGracefully(httpResp)
				usage, viaErr := responsesViaChatCompletions(c, info, adaptor, request)
				if viaErr != nil {
					return viaErr
				}
				service.PostTextConsumeQuota(c, info, usage, nil)
