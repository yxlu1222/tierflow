...
type RelayInfo struct {
	...
	SubscriptionId int
	// SubscriptionBucket 标识本次扣费命中的桶("premium"/"basic",空=premium)。
	SubscriptionBucket string
	// TokenSubscriptionId:请求所用 API Key 绑定的订阅 id(套餐专用 Key),
	// 0 = 普通 Key。资金源由此决定(D9):>0 定向扣该订阅,否则走钱包。
	TokenSubscriptionId int
	// GateBucket:套餐门禁(middleware/subscription_gate.go)判定出的桶,空=门禁未介入。
	// 门禁按模型组 id 精确判定并可能主动降级换组,是本次请求的权威结果,
	// 计费层优先采信,不再按最终模型名反查(见 NewBillingSession)。
	GateBucket string
	// SettledTotalTokens:结算时的真实 token 总量(prompt+completion),
	// 供 basic 桶按 token 数补差。0 = 未知(按预扣量结算,不补差)。
	SettledTotalTokens int
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	info := &RelayInfo{
		...
		TokenSubscriptionId: common.GetContextKeyInt(c, constant.ContextKeyTokenSubscriptionId),
		GateBucket:          common.GetContextKeyString(c, constant.ContextKeySubscriptionBucket),
