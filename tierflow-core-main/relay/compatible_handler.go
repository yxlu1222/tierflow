...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if fixed := service.RepairToolCallIDs(request.Messages); fixed > 0 {
		logger.LogInfo(c, fmt.Sprintf("tool call normalize: repaired %d empty tool_call id field(s)", fixed))
	}
