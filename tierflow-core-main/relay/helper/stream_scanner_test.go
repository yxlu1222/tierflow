...
// TestMain 统一设定流式超时。
//
// 此前每个 setup 辅助函数都会写一次全局 constant.StreamingTimeout 并在
// t.Cleanup 中还原,而绝大多数用例是 t.Parallel() 的 —— 并行下各自「还原成
// 自己读到的旧值」会互相覆盖,race detector 可稳定复现,表现为随机某个用例
// 失败。除 Timeout 用例需要特殊值(它已声明为串行)外,全包共用同一个值,
// 因此在此设置一次即可,各处不再读写。
...
// StreamScannerHandler 会无条件新建 StreamStatus(stream_scanner.go 开头),
// 因此传入的既有状态——包括此前记录的错误——会被丢弃。
//
// 这与该函数的语义一致:每次流式请求都应从干净的状态开始统计,生产代码中
// 也没有任何调用方预置 StreamStatus。此用例与实现由同一提交引入,原先断言
// 「保留既有错误」,与实现相反,自落地起从未通过。此处改为断言实际契约。
