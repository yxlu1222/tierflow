...
import (
	...
	"github.com/Zer0Echo/tierflow-core/relay/reasonmap"
			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					default:
						...
						// Claude 只接受图片与 PDF 文档。纯文本按文本块内联;
						// 已知且明确不受支持的类型直接报错 —— 不能静默丢弃,
						// 否则模型会在看不到附件的情况下照常作答,用户毫无察觉。
						switch {
						...
						case strings.HasPrefix(mimeType, "image/"),
							mimeType == "" && mediaMessage.Type == dto.ContentTypeImageURL:
							// image_url 探测不出 MIME 时仍按 image 下发,保持改造前的行为,
							// 避免把原本能走通的图片链接判成不支持;
							// file 类型探测不出 MIME 则无从归类,落到 default 报错。
							claudeMediaMessages = append(claudeMediaMessages, dto.ClaudeMediaMessage{
								Type: "image",
								Source: &dto.ClaudeMessageSource{
									Type:      "base64",
						...
						default:
							if mimeType == "" {
								name := "(unnamed)"
								if f := mediaMessage.GetFile(); f != nil && f.FileName != "" {
									name = f.FileName
								}
								return nil, fmt.Errorf("cannot determine the type of attachment %s; claude only accepts images, PDF and text files", name)
							}
							return nil, fmt.Errorf("claude does not support attachments of type %q; only images, PDF and text files can be sent", mimeType)
						}
					}
				}
				...
				// Anthropic 拒绝 content 为空数组的消息(400 messages.N.content:
				// must not be empty)。内容全被过滤掉时补一个占位文本块,
				// 与上面 StringContent 为空时补 "..." 的处理保持一致。
