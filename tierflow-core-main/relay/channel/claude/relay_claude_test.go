...
// TestRequestOpenAI2ClaudeMessage_RejectsUnsupportedFileContent
// 附件无法归类时必须报错,不能静默丢弃后照常发出去 —— 否则模型在看不到附件的
// 情况下作答,用户完全无从察觉自己的文件没送到。
				...
				Content: []any{
					...
					dto.MediaContent{
						...
						File: &dto.MessageFile{
							// 刻意不带扩展名:filepath.Ext 为空 → 推不出 MIME,
							// 结果与宿主机的 /etc/mime.types 无关。
							FileName: "blob",
...
// TestRequestOpenAI2ClaudeMessage_NeverEmitsEmptyContent
// Anthropic 拒绝 content 为空数组的消息(400 messages.N.content: must not be empty),
// 内容全被过滤掉时必须补占位文本块。
func TestRequestOpenAI2ClaudeMessage_NeverEmitsEmptyContent(t *testing.T) {
	request := dto.GeneralOpenAIRequest{
		Model: "claude-3-5-sonnet",
		Messages: []dto.Message{
			{
	...
	require.True(t, ok)
	require.NotEmpty(t, content, "content must never be an empty array")
	require.Equal(t, "text", content[0].Type)
	require.NotNil(t, content[0].Text)
	require.Equal(t, "...", *content[0].Text)
}

// TestRequestOpenAI2ClaudeMessage_DataURLMimeWinsOverFilename
// data: URL 里显式声明的 MIME 优先于文件名后缀:扩展名与声明不符时(scan.doc 装
// 的其实是 PDF),必须按 PDF 走 document 块,而不是被判成 msword 后整个丢掉。
func TestRequestOpenAI2ClaudeMessage_DataURLMimeWinsOverFilename(t *testing.T) {
	request := dto.GeneralOpenAIRequest{
		Model: "claude-3-5-sonnet",
		Messages: []dto.Message{
			{
