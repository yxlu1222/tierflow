...
func setupModelListControllerTestDB(t *testing.T) *gorm.DB {
	...
	// RoutingProfile 是被测路径的惰性依赖:model_list 会经 GetRoutingAliasList
	// 读取它,缺表会让查询报错并使模型列表缺项。
	require.NoError(t, db.AutoMigrate(
		&model.User{}, &model.Channel{}, &model.Ability{}, &model.Model{},
		&model.Vendor{}, &model.RoutingProfile{},
	))
...
// withUpstreamModelsExposed 让模型列表返回上游真实模型。
//
// TierFlow 默认只对外暴露路由别名(auto…),ApplyRoutingModelVisibility 会
// 过滤掉所有非别名模型。这两个用例验证的是「计费配置缺失时是否过滤」,
// 与可见性策略无关,故显式开启该开关把被测维度隔离出来。
func withUpstreamModelsExposed(t *testing.T) {
	t.Helper()

	common.OptionMapRWMutex.Lock()
	// OptionMap 由 model.InitOptionMap 建立,单测未走该初始化时是 nil map,
	// 直接写入会 panic。
	if common.OptionMap == nil {
		common.OptionMap = make(map[string]string)
	}
	original, had := common.OptionMap["ExposeUpstreamModels"]
	common.OptionMap["ExposeUpstreamModels"] = "true"
	common.OptionMapRWMutex.Unlock()

	t.Cleanup(func() {
		common.OptionMapRWMutex.Lock()
		if had {
			common.OptionMap["ExposeUpstreamModels"] = original
		} else {
			delete(common.OptionMap, "ExposeUpstreamModels")
		}
		common.OptionMapRWMutex.Unlock()
	})
}
...
func TestListModelsTokenLimitIncludesTieredBillingModel(t *testing.T) {
	...
	// 即便走 token 模型白名单分支,ListModels 仍会先经 getModelListGroups
	// 查用户分组;缺少 DB 与用户会在用户缓存处解引用 nil 而 panic。
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&model.User{
		Id:       1002,
