...
func setupLogin(user *model.User, c *gin.Context) {
	// 该行可能尚未被迁移回填(回填只在 master 节点跑,worker 节点可能先于
	// master 开始服务)。此时就地补分配,否则用户会「登录成功但之后每个
	// 请求都 401」,且客户端拿不到 uid 也就无法自愈。
	if user.PublicUid() == "" {
		if err := model.EnsureUserUid(user); err != nil {
			common.SysLog("failed to assign uid on login for user " + strconv.Itoa(user.Id) + ": " + err.Error())
			common.ApiErrorI18n(c, i18n.MsgUserSessionSaveFailed)
			return
		}
	}
	...
	session := sessions.Default(c)
	// 内部自增 id 仍写入 session:全站的 c.GetInt("id") 与所有外键都依赖它。
	// uid 一并写入,供 authHelper 校验 TF-User 头时直接比对,避免每请求反查。
	session.Set("id", user.Id)
	session.Set("uid", user.PublicUid())
	...
	// 响应里只给 uid,不再回显内部自增 id。11 条登录路径都汇到这里。
	c.JSON(http.StatusOK, gin.H{
		"message": "",
		"success": true,
		"data": map[string]any{
...
func TransferAffQuota(c *gin.Context) {
	...
	if !operation_setting.GetQuotaSetting().EnableInvitationReward {
		common.ApiErrorI18n(c, i18n.MsgInvitationRewardDisabled)
		return
	}
...
func GetSelf(c *gin.Context) {
	...
	responseData := map[string]interface{}{
		...
		// inviter_id 是「另一个用户」的内部自增 id,前端仅管理端列表使用,
		// 用户侧无消费方,故不下发。
		"linux_do_id": user.LinuxDOId,
		"setting":     user.Setting,
		// 注册时间。User 模型的字段是 CreatedAt,但对外沿用前端与其余模型
		// 一致的 created_time 键名。此前白名单遗漏了它,个人资料页的
		// 「创建时间」一直是空的。
...
func GetUserModels(c *gin.Context) {
	...
	// TierFlow: 用户侧只见路由别名，上游模型默认隐藏(系统设置→模型可见性开关)
	models = model.ApplyRoutingModelVisibility(models)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		"data":    models,
...
func TopUp(c *gin.Context) {
	...
	if err != nil {
		if msgKey, ok := redeemErrorMessageKey(err); ok {
...
// redeemErrorMessageKey 把兑换的哨兵错误映射到具体文案。
//
// 历史上所有失败都被折叠成「兑换失败，请稍后重试」，i18n 里那批具体译文形同虚设。
// 订阅码的失败原因（套餐停用、已有更高档、次数超限）是用户可以据此行动的，
// 必须区分开，否则用户无从自助排障。
func redeemErrorMessageKey(err error) (string, bool) {
	switch {
	case errors.Is(err, model.ErrRedemptionInvalid):
		return i18n.MsgRedemptionInvalid, true
	case errors.Is(err, model.ErrRedemptionUsed):
		return i18n.MsgRedemptionUsed, true
	case errors.Is(err, model.ErrRedemptionDisabled):
		return i18n.MsgRedemptionDisabled, true
	case errors.Is(err, model.ErrRedemptionExpired):
		return i18n.MsgRedemptionExpired, true
	case errors.Is(err, model.ErrRedemptionPlanMissing):
		return i18n.MsgRedemptionPlanMissing, true
	case errors.Is(err, model.ErrRedemptionPlanDisabled):
		return i18n.MsgRedemptionPlanDisabled, true
	case errors.Is(err, model.ErrSubscriptionUpgradeOnly):
		return i18n.MsgRedemptionUpgradeOnly, true
	case errors.Is(err, model.ErrSubscriptionNotUpgradable):
		return i18n.MsgRedemptionNotUpgradable, true
	case errors.Is(err, model.ErrSubscriptionPurchaseLimitReached):
		return i18n.MsgRedemptionLimitReached, true
	case errors.Is(err, model.ErrRedeemFailed):
		return i18n.MsgRedeemFailed, true
	}
	return "", false
}
