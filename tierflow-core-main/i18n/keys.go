...
const (
	...
	// 订阅码特有的失败原因（用户可据此行动，不能折叠成笼统的「兑换失败」）
	MsgRedemptionPlanMissing   = "redemption.plan_missing"
	MsgRedemptionPlanDisabled  = "redemption.plan_disabled"
	MsgRedemptionPlanRequired  = "redemption.plan_required"
	MsgRedemptionUpgradeOnly   = "redemption.upgrade_only"
	MsgRedemptionNotUpgradable = "redemption.not_upgradable"
	MsgRedemptionLimitReached  = "redemption.limit_reached"
	MsgRedemptionTypeInvalid   = "redemption.type_invalid"
	MsgRedemptionUsedImmutable = "redemption.used_immutable"

	// 已兑换的码不允许被改回可用状态（会让同一张码被兑换第二次）
	MsgRedemptionUsedStatusImmutable = "redemption.used_status_immutable"
)
...
const (
	...
	MsgInvitationRewardDisabled  = "invitation.reward_disabled"
)
