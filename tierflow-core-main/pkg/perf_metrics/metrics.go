...
import (
	...
	"github.com/Zer0Echo/tierflow-core/setting/perf_metrics_setting"
)
...
func Query(params QueryParams) (QueryResult, error) {
	...
	hotBuckets.Range(func(key, value any) bool {
		...
		// This query aggregates by (model, group); collapse the channel dimension.
		k.channelId = 0
...
// QueryChannelSummary returns, for one model, the aggregated performance of each
// channel that served it over the last `hours` (default 24h): persisted DB rows
// plus live hot buckets, summed per channel.
func QueryChannelSummary(modelName string, hours int) (ChannelSummaryResult, error) {
	if modelName == "" {
		return ChannelSummaryResult{ModelName: modelName}, nil
	}
	if hours <= 0 {
		hours = 24
	}
	...
	byChannel := map[int]counters{}
	rows, err := model.GetPerfMetricsChannelSummary(modelName, startTs, endTs)
	if err != nil {
		return ChannelSummaryResult{}, err
	}
	for _, row := range rows {
		byChannel[row.ChannelId] = counters{
			requestCount:   row.RequestCount,
			successCount:   row.SuccessCount,
			totalLatencyMs: row.TotalLatencyMs,
			ttftSumMs:      row.TtftSumMs,
	...
	hotBuckets.Range(func(key, value any) bool {
		...
		byChannel[k.channelId] = cur
		return true
	})

	channels := make([]ChannelSummary, 0, len(byChannel))
	for id, c := range byChannel {
		if c.requestCount == 0 {
			continue
		}
		channels = append(channels, ChannelSummary{
			ChannelId:    id,
			AvgLatencyMs: avg(c.totalLatencyMs, c.requestCount),
			AvgTtftMs:    avg(c.ttftSumMs, c.ttftCount),
			SuccessRate:  math.Round(successRate(c)*100) / 100,
			AvgTps:       math.Round(avgTps(c)*100) / 100,
	...
	return ChannelSummaryResult{ModelName: modelName, Channels: channels}, nil
}
