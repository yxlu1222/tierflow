...
func NotifyUser(userId int, userEmail string, userSetting dto.UserSetting, data dto.Notify) error {
	...
	case dto.NotifyTypeEmail:
		// 通知固定发往账号绑定邮箱。此前支持单独配置「通知邮箱」覆盖,
		// 但那只是一个几乎无人使用的可选项,已移除。
	...
	if system_setting.EnableWorker() {
		...
		workerReq := &WorkerRequest{
			...
			Headers: map[string]string{
				"User-Agent": "TierFlow-Bark-Notify/1.0",
			},
		}
		...
		// 设置User-Agent
		req.Header.Set("User-Agent", "TierFlow-Bark-Notify/1.0")
	...
	if system_setting.EnableWorker() {
		...
		workerReq := &WorkerRequest{
			...
			Headers: map[string]string{
				...
				"User-Agent":   "TierFlow-Gotify-Notify/1.0",
			},
			Body: payloadBytes,
		}
		...
		req.Header.Set("User-Agent", "TierFlow-Gotify-Notify/1.0")
