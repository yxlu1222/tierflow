...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	// TotalTokens=0(上游超时未返回用量)时保持 0,资金源将按预扣量结算不补差。
	relayInfo.SettledTotalTokens = totalTokens
	...
	providerCost := resolveProviderCostFromParams(relayInfo, billingexpr.TokenParams{
		P:   float64(usage.InputTokens),
		C:   float64(usage.OutputTokens),
		Len: float64(usage.InputTokens),
	}, quota, usage.InputTokens, usage.OutputTokens, 0, 0)
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	providerCost := resolveProviderCostFromUsage(relayInfo, usage, false, quota)
	model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		ChannelId:          relayInfo.ChannelId,
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		delta := int64(quota)
		if delta != 0 {
			bucket := model.SubscriptionBucket(relayInfo.SubscriptionBucket)
			if err := model.PostConsumeUserSubscriptionDelta(relayInfo.SubscriptionId, bucket, delta); err != nil {
				return err
			}
