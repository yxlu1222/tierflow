...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	reResolveTieredSnapshotForChannel(ctx, relayInfo)
	...
	// basic 桶按 token 数结算,把真实总 token 带给资金源(SubscriptionFunding.Settle)。
	// TotalTokens=0(上游未返回用量)时保持 0,资金源将按预扣量结算不补差。
	relayInfo.SettledTotalTokens = summary.TotalTokens
	...
	// TierFlow 智能路由：日志"模型"列原样展示用户调用的路由别名(模型广场模型名，
	// 品牌前缀由别名本身决定，如 tierflow-codex)，而非改写后的上游真实模型。真实
	// 上游对普通用户隐藏、仅存入 other，供管理员侧"路由"角标展开查看（前端
	// formatModelName 按 isAdmin 决定是否展示）。
	if alias := ctx.GetString("auto_route_alias"); alias != "" {
		upstream := relayInfo.UpstreamModelName
		if upstream == "" {
			upstream = logModel // 兜底：未发生渠道映射时即路由选中的上游模型
		}
		other["auto_route_alias"] = alias
		other["auto_route_upstream"] = upstream
		if profile := ctx.GetString("auto_route_profile"); profile != "" {
			other["auto_route_profile"] = profile
		}
		// 路由元数据全量落盘：组名快照(看板聚合展示)、组 id(下钻/校验)、tier、
		// 多模态短路与推理服务降级标记(仅管理端角标展示)。
		if group := ctx.GetString("auto_route_group"); group != "" {
			other["auto_route_group"] = group
		}
		if groupId := ctx.GetInt("auto_route_group_id"); groupId > 0 {
			other["auto_route_group_id"] = groupId
		}
		other["auto_route_tier"] = ctx.GetInt("auto_route_tier")
		if ctx.GetBool("auto_route_multimodal") {
			other["auto_route_multimodal"] = true
		}
		if ctx.GetBool("auto_route_degraded") {
			other["auto_route_degraded"] = true
		}
		// 用户侧彻底抽象上游：移除渠道级映射暴露，避免在用户日志里泄漏真实上游
		delete(other, "is_model_mapped")
		delete(other, "upstream_model_name")
		logModel = alias // 原样使用别名作为日志模型名
	}

	providerCost := resolveProviderCostFromUsage(relayInfo, originUsage, summary.IsClaudeUsageSemantic, summary.Quota)
	model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		ChannelId:          relayInfo.ChannelId,
		PromptTokens:       summary.PromptTokens,
		CompletionTokens:   summary.CompletionTokens,
		Strategy:           logModel,
		ModelGroup:         ctx.GetString("auto_route_group"),
		...
		SubscriptionBucket: relayInfo.SubscriptionBucket,
	})
	gopool.Go(func() {
