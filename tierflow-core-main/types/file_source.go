...
// DataURLMimeType 取出 data: URL 头部声明的 MIME(如 "data:application/pdf;base64,..."
// → "application/pdf")。不是 data: URL 或头部没声明 MIME 时返回空串。
//
// 客户端在 data: URL 里显式声明的 MIME 是权威值,优先级高于任何由文件名后缀
// 之类推导出来的猜测值。
func DataURLMimeType(data string) string {
	...
	// 头部形如 "<mime>;base64" 或 "<mime>";截到第一个 ';' 为止。
