...
// OverwriteRequestBody 用新内容替换已缓存的请求体存储。
// 供在 Distribute 之前改写请求体的中间件使用（如智能路由改写 model 字段），
// 替换后下游 GetBodyStorage / UnmarshalBodyReusable 读到的就是新内容。
func OverwriteRequestBody(c *gin.Context, body []byte) error {
	storage, err := CreateBodyStorage(body)
	if err != nil {
		return err
	}
	// 关闭并丢弃旧的缓存（存储 + 旧式 []byte 缓存）
