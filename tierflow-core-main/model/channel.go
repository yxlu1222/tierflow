...
// GetNextEnabledKeyExcluding is GetNextEnabledKey with support for skipping key
// indexes already tried (and failed) on the current request. This powers
// single-request key rotation: a KEY-scoped failure retries another untried key
// of the same channel before failing over to a different channel. Passing a nil
// exclude set makes it identical to GetNextEnabledKey. When every enabled key is
// excluded it returns ErrorCodeChannelNoAvailableKey, signalling that the
// channel's keys are exhausted for this request (caller should switch channels).
func (channel *Channel) GetNextEnabledKeyExcluding(exclude map[int]bool) (string, int, *types.NewAPIError) {
	...
	// Collect indexes of enabled keys, skipping any already tried this request.
	...
	// If no specific status list or none enabled (after excluding tried keys),
	// return an explicit error so the caller can switch channels. Returning the
	// first key here caused requests to keep using an already-disabled/tried key.
	if len(enabledIdx) == 0 {
		return "", 0, types.NewError(errors.New("no enabled keys"), types.ErrorCodeChannelNoAvailableKey)
	}

	// Prefer keys not cooling down in the routehealth breaker. If every enabled
	// key is cooling, fall back to the full enabled set (trying a cooling key
	// beats failing the request), and use a membership set so polling honours it.
	availIdx := make([]int, 0, len(enabledIdx))
	for _, idx := range enabledIdx {
		if routehealth.IsAvailable(channel.Id, idx) {
			availIdx = append(availIdx, idx)
		}
	}
	if len(availIdx) == 0 {
		availIdx = enabledIdx
	}
	availSet := make(map[int]bool, len(availIdx))
	for _, idx := range availIdx {
		availSet[idx] = true
	}

	switch channel.ChannelInfo.MultiKeyMode {
	case constant.MultiKeyModeRandom:
		// Randomly pick one breaker-available key
		selectedIdx := availIdx[rand.Intn(len(availIdx))]
	...
	case constant.MultiKeyModePolling:
		...
		for i := 0; i < len(keys); i++ {
			...
			if getStatus(idx) == common.ChannelStatusEnabled && availSet[idx] {
		...
		return keys[availIdx[0]], availIdx[0], nil
	default:
...
// HasUntriedEnabledKey reports whether the multi-key channel still has at least
// one enabled key whose index is not in exclude. Used by the relay retry loop to
// decide whether same-channel key rotation is still possible before switching
// channels. Read-only: it does not touch the polling index. Non-multi-key
// channels always return false (nothing to rotate to).
func (channel *Channel) HasUntriedEnabledKey(exclude map[int]bool) bool {
	if !channel.ChannelInfo.IsMultiKey {
		return false
	}
	keys := channel.GetKeys()
