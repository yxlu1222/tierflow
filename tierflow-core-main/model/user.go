...
type User struct {
	Id int `json:"id"`
	// Uid 是对外展示的用户标识:12 位纯数字、随机生成、与自增主键无关联。
	// 内部一切引用(外键、session、c.GetInt("id"))仍用 Id,Uid 只用于对外展示
	// 与 TF-User 鉴权握手。指针类型使未回填的行为 NULL —— NULL 在
	// SQLite/MySQL/PostgreSQL 的唯一索引里互不冲突,因此列与唯一索引可与回填解耦。
	Uid              *string `json:"uid" gorm:"type:varchar(12);column:uid;uniqueIndex"`
	...
	// InviterUid 是邀请人的对外 uid,非数据库列,仅在管理端列表查询时批量填充,
	// 供前端展示(前端不展示内部自增 id,见 Uid 字段注释)。
	InviterUid  string         `json:"inviter_uid,omitempty" gorm:"-:all"`
...
// attachInviterUids 为一页用户批量填充 InviterUid(邀请人的对外 uid)。
// 单次 IN 查询、无 N+1;带 Unscoped 使已软删的邀请人仍可解析。
func attachInviterUids(users []*User) {
	...
	var inviters []*User
	if err := DB.Unscoped().Select("id", "uid").Where("id IN ?", inviterIds).Find(&inviters).Error; err != nil {
		common.SysLog("failed to load inviter uids: " + err.Error())
		return
	}
	uidById := make(map[int]string, len(inviters))
	for _, inv := range inviters {
		uidById[inv.Id] = inv.PublicUid()
	}
	for _, u := range users {
		if u.InviterId > 0 {
			u.InviterUid = uidById[u.InviterId]
		}
	}
}
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int) ([]*User, int64, error) {
	...
	// 管理端展示的是对外 uid(见 User.Uid 注释),支持按 uid 精确检索。
	// 用原始字符串比较而非 keywordInt,避免 varchar 与整数跨库比较的差异。
	if IsValidUidFormat(keyword) {
		likeCondition = "uid = ? OR " + likeCondition
...
func (user *User) Insert(inviterId int) error {
	...
	if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() && operation_setting.GetQuotaSetting().EnableInvitationReward {
