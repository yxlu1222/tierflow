...
type Pricing struct {
	...
	// TierFlow: 路由别名(如 auto)行专用 —— 卡片不展示单一价格，描述用 profile 说明，
	// 详情展示各档位模型的定价。
	IsRoutingAlias bool               `json:"is_routing_alias,omitempty" gorm:"-"`
	RoutingTiers   []RoutingTierEntry `json:"routing_tiers,omitempty" gorm:"-"`
	// RoutingName/ScoreBands: 模型广场「路由策略/阈值配置」区块展示用。ScoreBands 为
	// 0-10 分数档位映射(如 "0-3:5,3-5:4,..."), 前端归一化到 0-100 渲染档位条。
	RoutingName string `json:"routing_name,omitempty" gorm:"-"`
	ScoreBands  string `json:"score_bands,omitempty" gorm:"-"`
	// RoutingKeywords: profile 关键词(分号分隔)，模型广场卡片下方展示。
	RoutingKeywords string `json:"routing_keywords,omitempty" gorm:"-"`
	// RoutingAliases: 该 profile 的全部调用别名(逗号分隔)，卡片展示为"调用名"。
	RoutingAliases string `json:"routing_aliases,omitempty" gorm:"-"`
}

// RoutingTierEntry 路由别名详情里的一个档位 = tier 序号 + 模型名 + 该档模型的定价(可能未配)。
type RoutingTierEntry struct {
	Tier      int      `json:"tier"`
	ModelName string   `json:"model_name"`
