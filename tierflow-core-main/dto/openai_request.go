...
func (m *MediaContent) ToFileSource() types.FileSource {
	...
	case ContentTypeFile:
		...
		// 由文件名扩展名推导 MIME:下游(如 Claude)靠它区分 document 与
		// image,留空会让所有文件一律被当作图片处理。
		//
		// 只在 data: URL 自己没声明 MIME 时才用文件名兜底——扩展名是猜测,
		// 客户端显式声明的才是权威值。反过来覆盖会让
		// {filename:"scan.doc", file_data:"data:application/pdf;base64,..."}
		// 被当成 msword,PDF 在下游被整个丢弃。
		mimeType := ""
		if types.DataURLMimeType(file.FileData) == "" {
