...
const CHANNEL_TYPE_DISPLAY_ORDER: number[] = [
  ...
  18, 45, 31, 35, 49, 19, 47, 37, 38, 39, 11, 8, 57, 22, 21, 44, 53, 56,
]

// 已停用的图像/视频/音乐生成类渠道类型:仅从"新建渠道"下拉隐藏,
// 后端数字常量与 CHANNEL_TYPES 显示映射仍保留,存量渠道记录不受影响。
// 36 SunoAPI, 50 Kling, 51 Jimeng, 52 Vidu, 54 DoubaoVideo, 55 Sora
const HIDDEN_CHANNEL_TYPES = new Set<number>([36, 50, 51, 52, 54, 55])

export const CHANNEL_TYPE_OPTIONS: { value: number; label: string }[] = (() => {
  const ordered: { value: number; label: string }[] = []
  const seen = new Set<number>()
  for (const id of CHANNEL_TYPE_DISPLAY_ORDER) {
    if (HIDDEN_CHANNEL_TYPES.has(id)) continue
  ...
  for (const [key, label] of Object.entries(CHANNEL_TYPES)) {
    const id = Number(key)
    if (id !== 0 && !HIDDEN_CHANNEL_TYPES.has(id) && !seen.has(id)) {
      ordered.push({ value: id, label })
    }
  }
  return ordered
