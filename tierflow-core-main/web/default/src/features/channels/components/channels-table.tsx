...
export function ChannelsTable() {
  ...
  // Live circuit-breaker health per channel (best-effort). Refreshed on a
  // gentle interval so cooldowns tick down without hammering the table.
  ...
  const healthByChannel = useMemo(() => {
    ...
    for (const p of healthData?.data?.providers ?? []) {
  ...
  const columns = useChannelsColumns(healthByChannel)
  ...
  return (
    <DataTablePage
      ...
      tableClassName={cn(
        'overflow-x-auto',
        // Unified single-card look shared with the keys / usage-log / models
        // tables: one uniform 14px body size across header + cells (overriding
        // the per-cell text-xs on badges/dates), unbolded headers, roomier rows.
