...
import { combineBillingExpr } from '@/lib/billing-expr'
import {
...
> & {
  className?: string
  /**
   * Embedded mode: hides the panel's own header, name field and footer so the
   * host (e.g. the model edit dialog) can provide a unified save button. The
   * host reads the assembled data imperatively via the exposed ref.
   */
  embedded?: boolean
  /**
   * When embedded, the model name is supplied by the host form instead of the
   * panel's own name input.
   */
  nameOverride?: string
}

/** Imperative handle exposed by {@link ModelPricingEditorPanel}. */
export type ModelPricingEditorHandle = {
  /**
   * Validates the current pricing form and returns the assembled data, or
   * `null` if validation fails (errors are surfaced inside the panel).
   */
  submit: () => Promise<ModelRatioData | null>
}
...
>(function ModelPricingEditorPanel(
  ...
  // In embedded mode the model name is owned by the host form; mirror it into
  // the panel's form so the preview, tiered editor and validation stay correct.
  useEffect(() => {
    if (embedded && nameOverride !== undefined) {
      form.setValue('name', nameOverride)
    }
  }, [embedded, nameOverride, form])
  ...
  const buildValidatedData = useCallback(
    (values: ModelPricingFormValues): ModelRatioData | null => {
      if (
        pricingMode === 'per-token' &&
        toNumberOrNull(promptPrice) === null &&
        laneConfigs.some(
      ...
      const resolvedName = (nameOverride ?? values.name).trim()
  ...
  const handleSubmit = (values: ModelPricingFormValues) => {
    // In embedded mode the host triggers save via the imperative ref; ignore
    // native form submits (e.g. Enter key) to avoid firing onSave twice.
    if (embedded) return

    const data = buildValidatedData(values)
    if (!data) return
  ...
  useImperativeHandle(
    ...
    () => ({
      submit: async () => {
        // In embedded mode the model name comes from the host form, so validate
        // the pricing fields only and skip the panel's own name validation.
        const valid = embedded
          ? await form.trigger(['price', 'ratio', 'completionRatio'])
          : await form.trigger()
        if (!valid) return null
        return buildValidatedData(form.getValues())
      },
    }),
    // buildValidatedData is memoized over the pricing state it reads, so the
    // handle recreates whenever those inputs change and submit() sees fresh values.
    [embedded, form, buildValidatedData]
  )
  ...
  return (
    <div
      className={cn(
        ...
        !embedded && 'bg-background rounded-xl border',
        className
      )}
    >
      {!embedded && (
                ...
                <TabsContent value='per-token' className='flex flex-col gap-5'>
                  <FieldGroup>
                    <Field>
                      ...
                      />
                      <FieldDescription>
                        {t('Price in CNY per 1M input tokens.')}
                      </FieldDescription>
                    </Field>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t(
                            'Price in CNY per request, regardless of tokens used.'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
...
export function PriceInput(props: {
  ...
  // Rule 8(CLAUDE.md):全站单币种,一律 ¥,禁止出现 $/USD。
  return (
    <InputGroup>
      <InputGroupAddon>¥</InputGroupAddon>
      <InputGroupInput
...
export function PriceLane(props: {
  ...
  enabled: boolean
  disabled?: boolean
  /** 启用态下输入框下方的提示;缺省为价格措辞(成本编辑器传入成本措辞) */
  enabledHint?: string
  ...
  return (
    ...
    >
      ...
      <p className='text-muted-foreground text-xs'>
        {props.enabled
          ? (props.enabledHint ?? t('Price in CNY per 1M tokens.'))
