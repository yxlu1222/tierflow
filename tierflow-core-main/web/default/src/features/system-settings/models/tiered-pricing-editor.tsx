...
// Rule 8(CLAUDE.md):全站单币种 ¥,表达式系数为 ¥/1M tokens。
...
function DraftNumberInput({
  ...
  // Sync the draft from the external value while the field isn't being edited
  // (and normalize it on blur). Done during render — React's "adjust state on
  // prop change" pattern — instead of in an effect, which would cascade renders.
  const [lastSync, setLastSync] = useState({ focused, value })
  if (lastSync.focused !== focused || lastSync.value !== value) {
    setLastSync({ focused, value })
    if (!focused) {
...
function VisualTierCard({
  ...
  // Keep the media section open once media pricing appears. Handled during
  // render (on the hasMediaPricing false→true transition) rather than in an
  // effect to avoid a cascading re-render.
  const [prevHasMedia, setPrevHasMedia] = useState(hasMediaPricing)
  if (prevHasMedia !== hasMediaPricing) {
    setPrevHasMedia(hasMediaPricing)
...
function PresetSection({ applyPreset, hideRequestRules }: PresetSectionProps) {
  const { t } = useTranslation()
  const [expanded, setExpanded] = useState(false)
  // 成本表达式场景(hideRequestRules)不支持 ||| 规则,整组围绕请求规则的预设
  // (组内预设全部带 requestRules)直接隐藏,避免应用后规则被静默丢弃。
  const groups = useMemo(
    () =>
      hideRequestRules
        ? PRESET_GROUPS.filter((g) =>
            g.presets.some((p) => !p.requestRules?.length)
          )
        : PRESET_GROUPS,
    [hideRequestRules]
  )
...
Numbers in the expression are CNY (¥) per 1M tokens prices. For example, p * 2.5 means input ¥2.50/1M tokens.
...
5. Price coefficients are CNY (¥) per 1M tokens prices (convert the provider's official price to CNY if it is published in another currency)
...
export type TieredPricingEditorProps = {
  ...
  /**
   * 隐藏请求规则(||| 倍率)编辑区。上游成本表达式复用本编辑器,但成本引擎
   * (provider_cost_setting)整串编译、不支持 ||| 规则,该场景下必须隐藏。
   */
  hideRequestRules?: boolean
}
...
export const TieredPricingEditor = memo(function TieredPricingEditor({
  ...
  const effectiveExpr = useMemo(() => {
    ...
    // hideRequestRules(成本表达式)场景:整串就是表达式本体,不做规则拆分——
    // 成本引擎整串编译,'(base) * (cond ? X : 1)' 这类倍率因子是合法成本
    // 表达式;若照常拆分,倍率段会被路由到 no-op 回调而在保存时静默丢失。
    if (hideRequestRules) {
      return rawExpr.trim()
    }
    const { billingExpr } = splitBillingExprAndRequestRules(rawExpr)
    return billingExpr
  }, [editorMode, visualConfig, rawExpr, hideRequestRules])
  ...
  const handleRawChange = useCallback(
    (value: string) => {
      setRawExpr(value)
      if (hideRequestRules) return // 整串即表达式,无规则半区可拆
      const { requestRuleExpr: ruleStr } =
        splitBillingExprAndRequestRules(value)
      onRequestRuleExprChange(ruleStr)
    },
    [onRequestRuleExprChange, hideRequestRules]
  )

  const handleModeChange = useCallback(
    (next: EditorMode) => {
      if (next === 'visual') {
        // hideRequestRules 场景整串即表达式,不拆规则半区(拆分会把倍率因子
        // 送进 no-op 回调造成丢失)。
        const { billingExpr, requestRuleExpr: ruleStr } = hideRequestRules
          ? { billingExpr: rawExpr.trim(), requestRuleExpr: '' }
  ...
  const applyPreset = useCallback(
    (preset: Preset) => {
      // 隐藏规则区时(成本表达式场景)丢弃预设自带的请求规则,避免拼出 ||| 段。
      const presetGroups = hideRequestRules ? [] : preset.requestRules || []
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex flex-wrap items-start gap-x-4 gap-y-1'>
        <div className='flex-1'>
          <PresetSection
            ...
            hideRequestRules={hideRequestRules}
          />
        </div>
      ...
      <div className='bg-muted/30 space-y-3 rounded-md border p-3'>
        ...
        {editorMode === 'visual' && !hideRequestRules && (
