...
// Update the request base URL (system option `ApiRequestAddress`). Independent
// from the system `ServerAddress` (which drives OAuth callbacks, payment return
// URLs, etc.); when unset, the keys page falls back to `ServerAddress`.
// Admin-only on the backend; reuses the shared option-update endpoint.
export async function updateApiRequestAddress(
  value: string
): Promise<{ success: boolean; message?: string }> {
  const res = await api.put('/api/option/', { key: 'ApiRequestAddress', value })
  return res.data
}
