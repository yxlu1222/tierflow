...
export function normalizeVisualTier(
  ...
  return {
    // Spread raw first so any extra passthrough fields survive, then let the
    // sanitized fields below win. (Previously `...tier` sat *after* the
    // sanitized fields and silently re-overrode cache_mode/conditions/costs
...
export function generateExprFromVisualConfig(
  ...
  for (let i = 0; i < tiers.length; i++) {
    ...
    // The last tier is always the else branch (default), so its condition is
    // ignored. A non-last tier with no usable condition matches everything and
    // therefore also terminates the chain as the else branch — emitting a bare
    // body mid-chain would produce an invalid ternary like `a ? b : c : d`.
    const cond = isLast ? '' : buildConditionStr(tier.conditions)
